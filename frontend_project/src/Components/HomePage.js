...
export default function HomePage() {
  ...
  const {setlogtype,setloggedin,loggedin,logtype,setuserdetails} = useStateContext()
  const logout = ()=>{
    setlogtype('')
    setloggedin(false)
    setuserdetails({})
  ...
  const emailforward = ()=> {
    ...
    <div className='home_body'>
        ...
        <div className='home_navlinks'>
          {loggedin && <Link smooth={true} offset={-100} className='home_links' onClick={dash}>DashBoard</Link>}
          ...
          { loggedin && <Link smooth={true} offset={-100} className='home_links' onClick={logout}>LogOut</Link>}
          { !loggedin && <Link smooth={true} offset={-100} className='home_links' onClick={()=>{navigate("/login")}}>Login</Link>}
        </div>
      </div>
