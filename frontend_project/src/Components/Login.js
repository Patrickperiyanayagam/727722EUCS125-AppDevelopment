...
export default function Login() {
    ...
    const {setuserdetails,setloggedin,setlogtype} = useStateContext();
...
function nextpage()
{
    ...
    else if(type === 'Transport Provider'){
        setlogtype('Transport Provider');
        ...
        .catch(err=>{console.log(err)})
            ...
            .then(res=>{
                if(res.data.length>0){
                    ...
                    toast.error("Login Unsuccessful\nInvalid Admin Credentials",{duration:1000})
                }
        })
        .catch(err=>{console.log(err)})
}
        else if(type === 'Transport Provider'){
            axios.get(`http://localhost:8080/tprovider/validate/${email}/${password}`)
            .then(res=>{
                if(res.data.length>0){
                    console.log(res.data[0])
                    setuserdetails(res.data[0])
